...
describe('check the order in BO', function () {
    ...
    it('create_order', function (done) {
        global.fctname = this.test.title;
        this.client
            .waitForExist(this.selector.BO.AddProductPage.new_order_button, 60000)
            .click(this.selector.BO.AddProductPage.new_order_button)
            .waitForExist(this.selector.BO.AddProductPage.search_customer_input, 60000)
            .setValue(this.selector.BO.AddProductPage.search_customer_input, 'john')
            .waitForExist(this.selector.BO.AddProductPage.new_client_choose_button, 60000)
            .click(this.selector.BO.AddProductPage.new_client_choose_button)
            .waitForExist(this.selector.BO.AddProductPage.search_product_input, 60000)
            .setValue(this.selector.BO.AddProductPage.search_product_input, 'dress')
            .waitForExist(this.selector.BO.AddProductPage.new_product_name_list, 60000)
            .isExisting(this.selector.BO.AddProductPage.new_product_combination_list).then(function (isExisting) {
    ...
    it('save_informations', function (done) {
        global.fctname = this.test.title;
        if (combination == true) {
            this.client.getText(this.selector.BO.AddProductPage.new_product_combination_choose).then(function (text) {
                var idx = text.lastIndexOf("-");
                global.product_combination = text.slice(0, idx);
                global.product_price = text.split("-").pop(-1);
            });
            this.client.getText(this.selector.BO.AddProductPage.new_product_name_choose).then(function (text) {
