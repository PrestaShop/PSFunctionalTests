...
describe('check the order in BO', function(){
	...
	it('go_to_order', function(done){
		this.client
			...
			.waitForVisible(this.selector.orders_form, 5000)
			.call(done);
	});
	...
	it('create_order', function(done){
		this.client
			.waitForVisible(this.selector.new_order, 5000)
			.click2(this.selector.new_order)
			.waitForVisible(this.selector.new_order_client, 5000)
			.setValue2(this.selector.new_order_client, 'john')
			.waitForVisible(this.selector.new_order_client_choose, 5000)
			.click2(this.selector.new_order_client_choose)
			.waitForVisible(this.selector.new_order_product, 5000)
			.setValue2(this.selector.new_order_product, 'dress')
			.waitForVisible(this.selector.new_order_product_name_list, 5000)
	...
	it('check_order', function(done){
			...
			this.client
			.waitForVisible(my_selector, 5000)
			.click2(my_selector)
			.waitForVisible(this.selector.order_product_name, 5000)
